...
interface ScrollEventDao {
    ...
    fun getScrollEventsByTimeRange(
        startTime: Long, endTime: Long, limit: Int = 100
    ...
    fun getScrollStatsByTime(
        startTime: Long, endTime: Long, groupByHour: Boolean
    ...
    fun getAppScrollStats(
        startTime: Long, endTime: Long, limit: Int = 20
...
//
    @Query("SELECT SUM(scrollDistanceMeters) FROM scroll_events")
...
interface AppUsageSessionDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertSession(session: AppUsageSession): Long

    @Query(
        """
        INSERT INTO wake_count (id,wakeCount) VALUES(1,1)
        ON CONFLICT(id) DO
...
data class TimeBasedScrollStats(
    val timeGroup: String, val scrollCount: Int, val totalDistance: Float, val avgDistance: Float
)
...
data class HourlyUsageStats(
    val hour: String, val totalTime: Long, val sessionCount: Int
)
